...
def get_model(model_args, training_args, data_args, bnb_model_from_pretrained_args):
    ...
    if 'tucker_state_dict' in locals() and tucker_state_dict is not None:
        ...
        if missing_keys:
            rank0_print(f"Missing {len(missing_keys)} parameters (will be initialized):")
            for key in missing_keys[:5]:  # 只打印前5个
                rank0_print(f"  - {key}")
        if unexpected_keys:
            rank0_print(f"Unexpected {len(unexpected_keys)} keys in checkpoint:")
...
def apply_lora_to_model(model, training_args):
    ...
    if training_args.use_tucker_4d:
        ...
        tucker_config = {
            ...
            'init_scale': getattr(training_args, 'tucker_init_scale', 0.02),
            'use_orthogonal_reg': training_args.use_orthogonal_reg,
            'ortho_reg_weight': training_args.ortho_reg_weight,
        }
        
        # Get model dtype
        model_dtype = next(model.parameters()).dtype
        rank0_print(f"Model dtype: {model_dtype}")
        
        # Track Tucker-LoRA layers
        model.tucker_lora_layers = {}
        replaced_modules = []
        ...
        for name, module in model.named_modules():
            if isinstance(module, nn.Linear):
                # Check if this module should have Tucker-LoRA
                if any(target in name for target in target_modules):
                    ...
                    *parent_path, layer_name = name.split('.')
                    ...
                    # Log the module being replaced
                    rank0_print(f"Replacing {name}: in_features={module.in_features}, out_features={module.out_features}, dtype={module.weight.dtype}")
                    
                    # Replace with Tucker-4D-LoRA layer
                    tucker_lora_linear = Tucker4DLoRALinear(module, **tucker_config)
                    setattr(parent, layer_name, tucker_lora_linear)
                    
                    # Track Tucker-LoRA parameters
                    model.tucker_lora_layers[name] = tucker_lora_linear.lora_layer
        ...
        rank0_print(f"Applied 4D Tucker-LoRA to {len(model.tucker_lora_layers)} layers:")
        rank0_print(f"  - Scene num: {scene_num}, Env num: {env_num}")
        rank0_print(f"  - Tucker ranks: {tucker_ranks}")
        rank0_print(f"  - Orthogonal regularization: {training_args.use_orthogonal_reg}")
        rank0_print(f"Trainable params: {trainable_params:,} ({trainable_percentage:.2f}%)")

    else:
        target_modules = training_args.lora_target_modules.split(",")
        
        # Parse Tucker-LoRA specific configs
        num_experts = getattr(training_args, 'tucker_num_experts', 4)
        tucker_ranks = getattr(training_args, 'tucker_ranks', (16, 16, 8))
        if isinstance(tucker_ranks, str):
            tucker_ranks = tuple(map(int, tucker_ranks.split(',')))
        ...
        # Track Tucker-LoRA layers for separate saving
        model.tucker_lora_layers = {}
        replaced_modules = []
        
        # Set model to evaluation mode temporarily to avoid dropout issues
        was_training = model.training
        model.eval()
            ...
            if isinstance(module, nn.Linear):
                ...
                if any(target in name for target in target_modules):
                    ...
                    *parent_path, layer_name = name.split('.')
                    ...
                    # Replace with Tucker-LoRA layer
                    tucker_lora_linear = TuckerLoRALinear(module, **tucker_config)
                    setattr(parent, layer_name, tucker_lora_linear)
                    
                    # Verify dtype after replacement
                    lora_dtype = tucker_lora_linear.lora_layer.U1.dtype
                    rank0_print(f"  Tucker-LoRA layer dtype: {lora_dtype}")
        ...
        rank0_print(f"Setting up explicit LoRA layers...")
        rank0_print(f"Applied Tucker-LoRA to {len(model.tucker_lora_layers)} layers:")
        rank0_print(f"  - Num experts: {num_experts}")
        rank0_print(f"  - Tucker ranks: {tucker_ranks}")
        rank0_print(f"  - Target modules: {replaced_modules[:5]}..." if len(replaced_modules) > 5 else f"  - Target modules: {replaced_modules}")
        rank0_print(f"Trainable params: {trainable_params:,} ({trainable_percentage:.2f}%)")
        rank0_print(f"LoRA setup completed")
